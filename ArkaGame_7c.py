...
class Circle:
    ...
    def update(self):
        self.radius -= 1
        gfxdraw.filled_circle(screen, self.x, self.y, self.radius, (255,0,0))
...
def collision1():
    ...
    global diff, lives, stage, score, loop, game, randomstage, velocity, flux
...
def mainloop():
    global startx, mousedir, diff, game, velocity, flux, counter
    ...
    while loop2:
        ...
        if flux == 1:
            ...
            if circle.radius > 0:
                ...
                circle.x = randrange(180, 220)
                circle.y = randrange(180, 220)
                circle.radius = 6
        if score % 1000 == 0:
            velocity = 3
            times = pygame.time.get_ticks()
